...
rendermainPage = (products, item_eachPage, currentPage) => {
  ...
  handlePageNumber = (num) => {
    ...
    let currentPageIndicator = document.querySelector(
      ".movepage-" + currentPage
    );
    let pageIndicators = [
      ...document.querySelectorAll("#pagination-numbers div"),
    ];
...
filter = () => {
  ...
  type_clear = (a) => {
    ...
    for (i = 0; i < savingtype_choices.length; i++) {
      let temp = a.filter(function (ele) {
        return ele.type.includes(savingtype_choices[i]);
      });
    ...
    for (i = 0; i < savingprice_choices.length; i++) {
      let temp = a.filter(function (ele) {
        switch (savingprice_choices[i]) {
          ...
          case "2": {
            return ele.price <= 300 && ele.price >= 100;
          }
          case "3": {
            return ele.price <= 500 && ele.price >= 300;
          }
          case "4": {
  ...
  //===================các Th lọc====================
  if (savingtype_choices.length != 0 && savingprice_choices.length == 0) {
    products = type_clear(main);
  } else if (
    savingtype_choices.length == 0 &&
    savingprice_choices.length != 0
  ) {
